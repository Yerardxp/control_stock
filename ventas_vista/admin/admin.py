...
from sqlqueries import QueriesSQLite # Importación de consultas SQL
from datetime import datetime, timedelta
import csv
from pathlib import Path
import os # proporciona un conjunto de funciones que permiten interactuar con el sistema operativo 

# Cargar el archivo KV que contiene la interfaz gráfica
Builder.load_file('admin/admin.kv')

# Clase para la disposición y comportamiento de elementos seleccionables en un RecycleBoxLayout
class SelectableRecycleBoxLayout(FocusBehavior, LayoutSelectionBehavior,RecycleBoxLayout):
    touch_deselect_last = BooleanProperty(True) # Propiedad que determina si el último elemento debe deseleccionarse al tocar

# Clase base para etiquetas de productos seleccionables en la interfaz gráfica
class SelectableProductoLabel(RecycleDataViewBehavior, BoxLayout):
	index = None
	selected = BooleanProperty(False)
	selectable = BooleanProperty(True)
	 # Método que actualiza la vista de la etiqueta
	...
	# Método que maneja el toque en la etiqueta
	def on_touch_down(self, touch):
		if super(SelectableProductoLabel, self).on_touch_down(touch):
			return True
		if self.collide_point(*touch.pos) and self.selectable:
			return self.parent.select_with_touch(self.index, touch)
	# Método que aplica la selección en la etiqueta
...
# Clase base para etiquetas de usuarios seleccionables en la interfaz gráfica
...
# Clase principal para la gestión de la vista de administrador
